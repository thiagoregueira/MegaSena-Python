...
# lêr o arquivo excel mega_sena_asloterias_ate_concurso_2735_sorteio.xlsx
#df_excel = pd.read_excel('mega_sena_asloterias_ate_concurso_2735_sorteio.xlsx')
...
def insert_data(db):
    ...
    if cursor.fetchone() is None:
        cursor.execute('INSERT INTO mega_sena (Concurso, Data, Bola1, Bola2, Bola3, Bola4, Bola5, Bola6) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (concurso, data_sorteio, bolas[0], bolas[1], bolas[2], bolas[3], bolas[4], bolas[5]))
        db.commit()
    else:
...
st.markdown("<h1 style='text-align: center; color: #FFD700;'>Análise de dados da Mega Sena</h1>", unsafe_allow_html=True)
st.write('---')
st.write('#')
st.write('#')
...
def filtro_concurso():
    ...
    concurso_fim = st.sidebar.number_input('Concurso Final', min_value=concurso_inicio, max_value=df['Concurso'].max(), value=df['Concurso'].max())
...
with col1:
    ...
    st.dataframe(df_filtrado_datas.sort_values(by='Data', ascending=False), hide_index=True)
...
# transformar o df em um formato de tabela
df_filtrado_concursos_frequencia_bolas = df_filtrado_concursos_frequencia_bolas.melt(var_name='Frequência', value_name='Bola')
# agrupar as bolas e contar quantas vezes cada uma foi sorteada
df_filtrado_concursos_frequencia_bolas = df_filtrado_concursos_frequencia_bolas.groupby('Bola').count()
# ordenar pela frequência
df_filtrado_concursos_frequencia_bolas = df_filtrado_concursos_frequencia_bolas.sort_values(by='Frequência', ascending=False)
# resetar o index
...
with col3:
    ...
    st.dataframe(df_filtrado_concursos.sort_values(by='Concurso', ascending=False), hide_index=True)
...
with col5:
    ...
    st.write(f'A dezena {number1} foi sorteada {df_filtrado_datas_frequencia_bolas[df_filtrado_datas_frequencia_bolas["Bola"] == number1].iloc[0, 1]} vezes')
    

with col6:
    number2 = st.number_input("Insira sua 2º dezena", min_value=1, max_value=60)
    # de acordo com a dezena escolhida informar quantas vezes ela foi sorteada
    st.write(f'A dezena {number2} foi sorteada {df_filtrado_datas_frequencia_bolas[df_filtrado_datas_frequencia_bolas["Bola"] == number2].iloc[0, 1]} vezes')
    # informar também quantas vezes as duas dezenas foram sorteadas juntas
    st.write(f'As dezenas {number1} e {number2} foram sorteadas juntas {df_filtrado_datas[(df_filtrado_datas["Bola1"] == number1) & (df_filtrado_datas["Bola2"] == number2)].shape[0]} vezes')

with col7:
    number3 = st.number_input("Insira sua 3º dezena", min_value=1, max_value=60)
    # de acordo com a dezena escolhida informar quantas vezes ela foi sorteada
    st.write(f'A dezena {number3} foi sorteada {df_filtrado_datas_frequencia_bolas[df_filtrado_datas_frequencia_bolas["Bola"] == number3].iloc[0, 1]} vezes')
    # informar também quantas vezes as três dezenas foram sorteadas juntas
    st.write(f'As dezenas {number1}, {number2} e {number3} foram sorteadas juntas {df_filtrado_datas[(df_filtrado_datas["Bola1"] == number1) & (df_filtrado_datas["Bola2"] == number2) & (df_filtrado_datas["Bola3"] == number3)].shape[0]} vezes')

with col8:
    number4 = st.number_input("Insira sua 4º dezena", min_value=1, max_value=60)
    # de acordo com a dezena escolhida informar quantas vezes ela foi sorteada
    st.write(f'A dezena {number4} foi sorteada {df_filtrado_datas_frequencia_bolas[df_filtrado_datas_frequencia_bolas["Bola"] == number4].iloc[0, 1]} vezes')
    # informar também quantas vezes as quatro dezenas foram sorteadas juntas
    st.write(f'As dezenas {number1}, {number2}, {number3} e {number4} foram sorteadas juntas {df_filtrado_datas[(df_filtrado_datas["Bola1"] == number1) & (df_filtrado_datas["Bola2"] == number2) & (df_filtrado_datas["Bola3"] == number3) & (df_filtrado_datas["Bola4"] == number4)].shape[0]} vezes')

with col9:
    number5 = st.number_input("Insira sua 5º dezena", min_value=1, max_value=60)
    # de acordo com a dezena escolhida informar quantas vezes ela foi sorteada
    st.write(f'A dezena {number5} foi sorteada {df_filtrado_datas_frequencia_bolas[df_filtrado_datas_frequencia_bolas["Bola"] == number5].iloc[0, 1]} vezes')
    # informar também quantas vezes as cinco dezenas foram sorteadas juntas
    st.write(f'As dezenas {number1}, {number2}, {number3}, {number4} e {number5} foram sorteadas juntas {df_filtrado_datas[(df_filtrado_datas["Bola1"] == number1) & (df_filtrado_datas["Bola2"] == number2) & (df_filtrado_datas["Bola3"] == number3) & (df_filtrado_datas["Bola4"] == number4) & (df_filtrado_datas["Bola5"] == number5)].shape[0]} vezes')

with col10:
    number6 = st.number_input("Insira sua 6º dezena", min_value=1, max_value=60)
    # de acordo com a dezena escolhida informar quantas vezes ela foi sorteada
    st.write(f'A dezena {number6} foi sorteada {df_filtrado_datas_frequencia_bolas[df_filtrado_datas_frequencia_bolas["Bola"] == number6].iloc[0, 1]} vezes')
    # informar também quantas vezes as seis dezenas foram sorteadas juntas
    st.write(f'As dezenas {number1}, {number2}, {number3}, {number4}, {number5} e {number6} foram sorteadas juntas {df_filtrado_datas[(df_filtrado_datas["Bola1"] == number1) & (df_filtrado_datas["Bola2"] == number2) & (df_filtrado_datas["Bola3"] == number3) & (df_filtrado_datas["Bola4"] == number4) & (df_filtrado_datas["Bola5"] == number5) & (df_filtrado_datas["Bola6"] == number6)].shape[0]} vezes')
